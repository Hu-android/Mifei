...
public class AppUtils {
    ...
    /**
     * 获取 Manifest 中定义的 data 数据
     *
     * @param context
     * @param name    Manifest 中定义的名字
     * @return
     */
    @SuppressLint("NewApi")
    public static String getManifestData(Context context, String name) {
        try {
            ...
            return bundle.getString(name, DEFAULT_VALUE);
        } catch (Exception e) {
            e.printStackTrace();
     ...
     */
    @SuppressLint("NewApi")
    public static String getManifestData(Context context, String name, String defaulValue) {
        try {
            ApplicationInfo ai = context.getPackageManager().getApplicationInfo(context.getPackageName(), PackageManager.GET_META_DATA);
            Bundle bundle = ai.metaData;
            return bundle.getString(name, defaulValue);
        } catch (Exception e) {
            e.printStackTrace();
            return defaulValue;
        }
...
//        }
     ...
     * @param context 上下文
     */
    public static boolean cleanAppData(Context context) {
        boolean isSuccess = CleanUtils.cleanInternalCache(context);
        isSuccess &= CleanUtils.cleanInternalDbs(context);
        isSuccess &= CleanUtils.cleanInternalSP(context);
        isSuccess &= CleanUtils.cleanInternalFiles(context);
