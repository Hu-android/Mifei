...
public class NineGridView<T> extends ViewGroup {
    ...
    private int mdoubleImageSize = 200;   // 2张图片时的最大大小,单位dp
    ...
    public NineGridView(Context context, AttributeSet attrs) {
        ...
        this.mdoubleImageSize = typedArray.getDimensionPixelSize(R.styleable.NineGridView_doubleImgSize, -1);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mImgDataList != null && mImgDataList.size() > 0) {
            ...
            } else if (mImgDataList.size() == 2 && mdoubleImageSize != -1) {
                mGridSize = mdoubleImageSize > (totalWidth - mGap) / 2 ? (totalWidth - mGap) / 2 : mdoubleImageSize;
            } else {
    ...
    private void layoutChildrenView() {
        ...
        for (int i = 0; i < showCount; i++) {
            ...
            if (mAdapter != null) {
                mAdapter.onDisplayImage(getContext(), childrenView,i, mImgDataList.get(i));
            }
    ...
    public void setImagesData(List lists) {
        ...
        if (lists.size() > mMaxSize) {
            ...
            if (child instanceof NineGridViewWrapper) {
                ...
                imageView.setMoreNum(lists.size() - mMaxSize);
            }
        }
        mImgDataList = lists;
        requestLayout();
